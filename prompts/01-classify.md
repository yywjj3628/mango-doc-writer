...
- 宣传推文：用于公众号/传播类内容，强调传播性但遵守事实边界。区分 style_domain：dianguang_siqing 克制正式，mango_official_account 传播感强。
- 工作方案：用于未来工作安排、重点任务、责任分工、时间安排。区别于通知（布置当前任务）和汇报材料（向领导汇报）。
- 会议新闻：用于报道会议发生、主题、主要内容、部署要求、意义和下一步。领导讲话可选，不得编造。
- 经营月报：用于久之润月度经营情况总结，包含当月重点工作、经营数据、问题分析和下月计划。区别于通用汇报材料和年度总结。
- 理论学习发言：用于党委理论学习中心组研讨交流发言，围绕学习主题结合企业实际谈认识、体会和思考。区别于普通领导讲话（向下部署）和汇报材料（向上汇报）。
- 其他
...
- 素材有议定事项但用户要新闻稿 → 写会议新闻，议定事项作为内容
- 素材有召开/强调但用户要纪要 → 写会议纪要
...
### 产品介绍 / 内部讨论 / 口径打磨 vs 新闻稿（v0.1.4.2 新增）
...
如果 `specified_doc_type = 汇报材料`，且 `scene` 包含「内部讨论」「产品介绍」「口径打磨」或类似信号，**除非用户明确要求「对外新闻稿」**，否则不得改判为新闻稿。

不要仅因为语言中出现「传播口径」「年轻用户」「品牌表达」「用户价值」就判为新闻稿。这些表达在产品介绍和内部讨论材料中同样常见。
